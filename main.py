...
DEVICE_SESSION_V4_SCHEMA = "ylx.device-session.v4"
...
@lru_cache
def _device_session_v4_validator() -> Draft202012Validator:
    schema = json.loads((Path(__file__).parent / "openaria/bridge/sdk/schemas/ylx-device-session-v4.schema.json").read_text())
    ...
    def extend(value: Any) -> None:
        if isinstance(value, dict):
            if value.get("const") == DEVICE_SESSION_V2_SCHEMA:
                value.pop("const")
                value["enum"] = [DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA]
...
def _device_session_artifact(
    ...
    *,
    ...
    _require_device_session_v1(
        required.issubset(artifact) and set(artifact).issubset(required | {"storage_encoding"}),
        "artifact fields",
    )
    ...
    if expected_media_type is not None:
        _require_device_session_v1(
            media_type == expected_media_type or (
                (expected_media_type, media_type) in {
                    ("application/x-ndjson", "application/zstd"), ("audio/wav", "audio/flac")
                } and "storage_encoding" in artifact
            ), "artifact media_type"
        )
...
def _validate_device_session_v2_audio_invariants(manifest: dict[str, Any]) -> None:
    ...
    for expected_index, segment in enumerate(audio["segments"]):
        ...
        if artifact.get("media_type") != "audio/flac" and artifact["bytes"] != expected_file_bytes:
            raise PipelineError(
...
def _read_device_session_v1(
    ...
    _require_device_session_v1(set(manifest) - ({"capture_audit"} if manifest["schema"] == DEVICE_SESSION_V4_SCHEMA else set()) == expected_top, "top-level fields")
        ...
        "segments": [
            {
                ...
                "artifact": {
                    ...
                    **({"storage_encoding": segment["artifact"]["storage_encoding"]} if "storage_encoding" in segment["artifact"] else {}),
                },
            }
...
def _read_device_session_v2(
    ...
    _validate_schema(
        _device_session_v4_validator() if manifest["schema"] == DEVICE_SESSION_V4_SCHEMA else _device_session_v3_validator() if manifest["schema"] == DEVICE_SESSION_V3_SCHEMA else _device_session_v2_validator(),
    ...
    return Session(
        ...
        source_manifest_sha256=hashlib.sha256(manifest_bytes).hexdigest(),
        source_manifest_revision=f"sha256:{hashlib.sha256(manifest_bytes).hexdigest()}",
        source_signature={"status": "device_session_" + manifest["schema"].rsplit(".", 1)[1] + "_sealed"},
        source_manifest_name="manifest.json",
...
def _dispatch_root_manifest(
    ...
    if schema in {DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}:
...
def _validate_closed_device_session_take_graph(sessions: list[Session]) -> None:
    device_sessions = [
        ...
        in {DEVICE_SESSION_V1_SCHEMA, DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}
    ]
...
def _readable_session_takes(
    ...
    readable = [
        ...
        for session in sessions
        if session.source_manifest_schema
        not in {DEVICE_SESSION_V1_SCHEMA, DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}
    ]
...
def _validate_device_session_v2_audio_wav_bytes(
    ...
    if segment["artifact"].get("media_type") == "audio/flac":
        from openaria.bridge.sdk._storage import verify_flac
        try:
            verify_flac(raw, source_audio, segment)
        except (ValueError, subprocess.SubprocessError) as error:
            raise PipelineError(f"{label} FLAC validation failed: {error}") from error
        return
...
def _device_session_v2_audio_segments_by_path(
    ...
    if (
        session.source_manifest_schema not in {DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}
...
def export_sbs(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if session.source_manifest_schema in {DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}:
...
def _publication_source_audio(session: Session) -> dict[str, Any]:
    if session.source_manifest_schema not in {DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}:
...
def _validate_bucket_publication(
    ...
    if publication_schema == BUCKET_PUBLICATION_V2_SCHEMA:
        ...
        if expected_source_schema not in {DEVICE_SESSION_V2_SCHEMA, DEVICE_SESSION_V3_SCHEMA, DEVICE_SESSION_V4_SCHEMA}:
